...
local function compile()
  ...
  vim.system(
    { "typst", "compile", src, pdf, "--diagnostic-format", "short" },
    { text = true },
    function(res)
      vim.schedule(function()
        local qf = {}
        for line in vim.gsplit((res.stdout or "") .. "\n" .. (res.stderr or ""), "\n", { plain = true }) do
          if line ~= "" then table.insert(qf, parse_qf(line)) end
        end
...
local function start()
  ...
  vim.api.nvim_create_autocmd("BufWritePost", {
    group = group, pattern = "*.typ", callback = compile,
  })
  compile()
...
local function stop()
  ...
  if sioyek then
    sioyek:kill("sigterm"); sioyek = nil
  end
